...
| 跨源聚合 | 无 | 无 | wiki + doc（sheets/会议纪要 planned） |
...
### 已完成（2026-04-05 第四轮：输出体验 + 健壮性）

- [x] **终端输出增强（P2）** — 聚类展开文档标题（树形 ├/└/│）、碰撞洞察分行显示 reasoning
- [x] **飞书文档排版优化（P2）** — 概览 callout 卡片、章节分割线、碰撞 reasoning 独立 callout
- [x] **dotenv 自动加载（P3）** — Node 内置 `process.loadEnvFile()`，零依赖
